...
pipeline {
    ...
    stages {
        ...
        stage('Aprobacion PROD') {
            ...
            steps {
                input message: 'Confirmas deploy de notifications-api en PROD?', ok: 'Si, deployar'
            }
        }
                ...
                script {
                    ...
                    withCredentials([
                        ...
                        bat """
                            ...
                            -i "%SSH_KEY%" ^
                            ubuntu@%EC2_IP% ^
                            "if [ ! -d /home/ubuntu/devmart-infra ]; then cd /home/ubuntu && git clone -b ${INFRA_BRANCH} https://github.com/jsolano0112/devmart-infra.git; fi && cd /home/ubuntu/devmart-infra && docker-compose pull notifications-api-1 websocket-1 && docker-compose up -d notifications-api-1 websocket-1"
                        """
                    }
                }
            }
    ...
    post {
        success { echo "notifications-api desplegado en ${env.DEPLOY_TARGET == 'prod' ? 'PROD' : 'QA'}" }
        failure { echo 'Fallo el pipeline de notifications-api' }
    }
}
